...
## data loader ###
##################
# 🛑 change links to point to data files
test_images_data = h5py.File("link to image.h5", "r")
test_cloud_data = h5py.File("link to cloud.h5", "r")
test_scl_data = h5py.File("link to scl.h5", "r")
test_lat_data = h5py.File("link to lat.h5", "r")
test_lon_data = h5py.File("link to lon.h5", "r")
...
def create_features(img_data, scl_data, cld_data, lat_data, lon_data):
    """input data -> scaled image data"""
    images = np.array(img_data["images"])
    # Scale bands
    band_data = {}
...
def main():
    ...
    x_features = create_features(
        test_images_data, test_scl_data, test_cloud_data, test_lat_data, test_lon_data
    )
    np.savetxt("X_heldout.txt", x_features, delimiter="\t")
    print("Held out set successfully created!")

    # Update link to uniqiue ID pairs csv
    tile_id = pd.read_csv("link to Unique ID pairs.csv")
    tile_id = tile_id.sort_values(by=["S2_idx"])
    tile_id = tile_id.rename(columns={"ID": "Tile"})
    tile_id.to_csv("Tile_DS.csv", index=False)
